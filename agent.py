...
**قابلیت‌ها**: مدیریت رویدادها (ایجاد/جستجو با تاریخ، عنوان، مکان، شرکت‌کننده) و وظایف (ایجاد/جستجو/به‌روزرسانی با مهلت، توضیحات، پروژه، مسئول، وضعیت).
...
def check_approval_node(state: AgentState) -> AgentState:
    ...
    if hasattr(last_message, "tool_calls") and last_message.tool_calls:
        ...
        approval_message = AIMessage(
            content=f"⚠️ **نیاز به تأیید**\n\nمن می‌خواهم این عملیات را انجام دهم: **{tool_name_display}**\n\n**مشخصات:**\n"
        )
        ...
        approval_message.content += "\n**آیا این عملیات را تأیید می‌کنید؟**\n"
...
def execute_approved_node(state: AgentState) -> AgentState:
    ...
    if is_approved and not is_rejected:
        ...
        mock_message = AIMessage(content="", tool_calls=[pending_action])
...
class IranianManagerAssistant:
    ...
    def chat(self, user_input: str, thread_id: str = "default") -> str:
        ...
        if current_state.values.get("pending_action"):
            # User is responding to approval request - execute the action
            ...
            if is_approved and not is_rejected:
                # Execute the tool directly by calling the tool function
                ...
                if tool_func:
                    try:
                        ...
                        result_content = f"✅ عملیات با موفقیت انجام شد!\n\n{result}\n\n🔄 (گفتگو پاکسازی شد - آماده دستور بعدی)"
                        
                        # CRITICAL FIX: After tool execution, we need to COMPLETELY CLEAR the checkpoint
                        # to prevent message history issues. We'll return a special marker.
                        return f"__CLEAR_THREAD__|{result_content}"
                    except Exception as e:
                        error_msg = f"❌ خطا در اجرای عملیات: {str(e)}\n\n🔄 (گفتگو پاکسازی شد)"
                        return f"__CLEAR_THREAD__|{error_msg}"
                else:
                    error_msg = "❌ ابزار مورد نظر یافت نشد.\n\n🔄 (گفتگو پاکسازی شد)"
                    return f"__CLEAR_THREAD__|{error_msg}"
            else:
